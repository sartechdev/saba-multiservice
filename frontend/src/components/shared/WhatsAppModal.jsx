...
export const WhatsAppModal = ({ isOpen, onClose, product = null, title = 'Consultar por WhatsApp', subtitle = null, customMessage = null }) => {
  ...
  // Link directo / universal para WhatsApp
  const waDirectUrl = `https://wa.me/${phone}?text=${encodedMessage}`;

  // QR Code URL mediante API pública de qrcode (Google Charts / api.qrserver.com)
  const qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?size=240x240&data=${encodeURIComponent(
    waDirectUrl
  )}&margin=10`;

  const displayTitle = title;
  const displaySubtitle = subtitle || (product ? product.name : 'Saba Multiservice Santa Fe');
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="wa-modal-overlay" onClick={onClose}>
          ...
          >
            <button className="wa-modal-close" onClick={onClose} aria-label="Cerrar modal">
              ✕
            </button>
            ...
            <div className="wa-modal-header">
              ...
              <h3 className="wa-modal-title">{displayTitle}</h3>
              <p className="wa-modal-product-name">{displaySubtitle}</p>
            </div>
            ...
            <div className="wa-modal-body">
              <div className="wa-qr-container">
                <img
                  src={qrCodeUrl}
                  alt={`QR WhatsApp - ${displaySubtitle}`}
                  className="wa-qr-image"
                />
                <span className="wa-qr-hint">📱 Escaneá con tu celular para abrir el chat</span>
              </div>

              <div className="wa-actions-stack">
                <a
                  href={waDirectUrl}
                  target="_blank"
